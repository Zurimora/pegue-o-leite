...
public class BacteriaSpawner : MonoBehaviour
{
    ...
    [SerializeField] GameObject bacteria1; //object to spawn
    ...
    IEnumerator Spawn()
    {
        while (true) //make spawning endless
        {
            ...
            GameObject bacteria = Instantiate(bacteria1, new Vector2(spawnPosX, 7f), Quaternion.identity); //spawning food object
            bacteria.GetComponent<SpriteRenderer>().sprite = sprites[Random.Range(0, sprites.Length)]; //chang sprite to random from array
            bacteria.GetComponent<Rigidbody2D>().velocity = new Vector2(0, velocity); //add velocity
    ...
    IEnumerator PeriodAddEverySec()
    {
        ...
        {
            if (spawnPeriod > 0.6f) //if period is greater than 0.5 subtract period
            {
